...
<div class="stats-grid" style="grid-template-columns: repeat(4, 1fr); gap: 15px; width: 85%; margin: 0 auto 25px auto;">
    <div class="stat-box">
    ...
    <div class="stat-box" style="background: #f0fdf4; border: 1px solid #bbf7d0;">
        <h4 style="color: #15803d;">💰 My Total Earnings</h4>
        <div class="count" id="stat-earnings" style="color: #16a34a; font-weight: bold;"><?php echo number_format($stats['earnings'], 2); ?> ৳</div>
        <small style="color: #166534; font-size: 11px;">(80 ৳ fee per delivery)</small>
    </div>
</div>
    ...
    <table class="data-table" style="width: 100%; border-collapse: collapse; text-align: left;">
        <thead>
            <tr style="background: #f8fafc; border-bottom: 2px solid #edf2f7;">
                ...
                <th style="padding: 12px;">Order Total & Payout</th>
        ...
        <tbody id="delivery-rows">
            <?php if ($deliveries && mysqli_num_rows($deliveries) > 0): ?>
                <?php while ($d = mysqli_fetch_assoc($deliveries)): ?>
                    <?php 
                        $isCustom = !empty($d['custom_order_id']);
                        $displayOrderId = $isCustom ? '#CR-' . $d['custom_order_id'] : '#ORD-' . $d['order_id'];
                        $itemName = $isCustom ? $d['custom_craft_name'] : ($d['store_product_name'] ?? 'Handmade Craft');
                        $itemImg = $isCustom ? ($d['custom_craft_image'] ?? 'sample1.jpg') : ($d['store_product_image'] ?? 'sample1.jpg');
                    ?>
                    <tr id="row-del-<?php echo $d['delivery_id']; ?>" style="border-bottom: 1px solid #eee;">
                        <td style="padding: 12px;">
                            <b style="color: <?php echo $isCustom ? '#9b2c2c;' : '#2b6cb0;'; ?>"><?php echo $displayOrderId; ?></b>
                            <?php if ($isCustom): ?>
                                <br><span style="background: #fed7d7; color: #9b2c2c; font-size: 10px; padding: 2px 5px; border-radius: 3px; font-weight: bold;">Custom Craft</span>
                            <?php endif; ?>
                        </td>
                        <td style="padding: 12px;">
                        ...
                        <td style="padding: 12px;">
                            <div style="display: flex; align-items: center; gap: 10px;">
                                <img src="../../assets/images/uploads/<?php echo htmlspecialchars($itemImg); ?>" style="width: 45px; height: 45px; object-fit: cover; border-radius: 4px; border: 1px solid #ddd;" onerror="this.src='../../assets/images/sample1.jpg';">
                                <div>
                                    <strong style="font-size: 13px;"><?php echo htmlspecialchars($itemName); ?></strong>
                                    <?php if ($isCustom): ?>
                                        <br><small style="color: #666; font-size: 11px;">Theme: <?php echo htmlspecialchars($d['custom_craft_theme']); ?> (<?php echo htmlspecialchars($d['custom_craft_size']); ?>)</small>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </td>
                        <td style="padding: 12px;">
                            ...
                            <small style="color: #666;"><?php echo htmlspecialchars($d['payment_gateway']); ?></small><br>
                            <span style="display: inline-block; margin-top: 3px; background: #e6fffa; color: #234e52; padding: 2px 6px; border-radius: 3px; font-size: 11px; font-weight: bold;">
                                Rider Share: 80.00 ৳
                            </span>
                        </td>
                        <td style="padding: 12px;">
